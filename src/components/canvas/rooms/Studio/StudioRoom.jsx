...
const StudioRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
  ...
  const responsiveParams = useMemo(() => {
    ...
    return {
      ...
      panRight: isMobile
        ...
        : isTablet
          ? 0.5
          : Math.max(0.3, (size.width / 1920) * CAMERA_PAN_RIGHT),
  ...
  const monitorData = useMemo(() => {
    ...
    for (
      let ring = 0;
      ring < ringsNeeded && contentIndex < shuffledContent.length;
      ring++
    ) {
      ...
      for (
        let i = 0;
        i < MONITORS_PER_RING && contentIndex < shuffledContent.length;
        i++
      ) {
        ...
        const yJitter =
          (Math.sin(contentIndex * 1.7) + Math.cos(contentIndex * 2.3)) * 0.4; // Pseudo-random
        ...
        switch (platform.shape) {
          case "tv":
            width = 1.6;
            height = 1.187;
            depth = 1.0; // Legacy 1.348 ratio
            break;
          case "monitor":
            width = 1.6;
            height = 1;
            depth = 0.15; // Legacy 1.835 ratio
            break;
          case "phone":
            width = 0.6;
            height = 1.139;
            depth = 0.1; // Legacy 0.527 ratio
            break;
          default:
            width = 1.4;
            height = 1.0;
    ...
    const minBaseY =
      items.length > 0 ? Math.min(...items.map((m) => m.baseY)) : 0;
    const maxBaseY =
      items.length > 0 ? Math.max(...items.map((m) => m.baseY)) : 0;
    // Make sure we have a baseline height so monitors don't instantly teleport if there's only 1 row
    const totalHeight = Math.max(
      VERTICAL_SPACING * 3,
      maxBaseY - minBaseY + VERTICAL_SPACING,
    );
    ...
    (item) => {
      ...
      gsap.to(towerRef.current.rotation, {
        ...
        onComplete: () => {
          ...
          const monitorCurrentY =
            item.baseY + (monitorOffsets.current[item.index] || 0);
          const monitorWorldY =
            -1.2 + monitorCurrentY + responsiveParams.yOffset;
          ...
          const targetX =
            camera.position.x + forward.x * zoomDist + right.x * panRight;
          const targetZ =
            camera.position.z + forward.z * zoomDist + right.z * panRight;
  ...
  const handleReturnCamera = useCallback(() => {
    ...
    if (
      originalCameraX.current !== null &&
      originalCameraY.current !== null &&
      originalCameraZ.current !== null
    ) {
      gsap.to(camera.position, {
  ...
  useFrame((state, delta) => {
    ...
    if (!isDraggingRef.current && !isAnimating && !selectedMonitor) {
      towerRef.current.rotation.y +=
        autoRotationSpeed.current * delta + rotationVelocity.current;
      ...
      const targetDrift =
        fallSpeed.current > 0 ? BASE_FALL_SPEED : -BASE_FALL_SPEED;
      fallSpeed.current = THREE.MathUtils.lerp(
        fallSpeed.current,
        targetDrift,
        1.0 - SPEED_DECAY,
      );
    ...
    <group ref={groupRef} position={[0, -1.2, 0]}>
      ...
      >
        ...
        <mesh visible={false}>
          <cylinderGeometry
            args={[
              responsiveParams.towerRadius + 0.5,
              responsiveParams.towerRadius + 0.5,
              TOWER_HEIGHT * 1.5,
              16,
            ]}
...
const MonitorBlock = memo(
  ({
    ...
    const monitorBack = useLoader(
      TextureLoader,
      "/textures/studio/monitor_back.webp",
    );
    const monitorTop = useLoader(
      TextureLoader,
      "/textures/studio/monitor_top.webp",
    );
    const monitorBottom = useLoader(
      TextureLoader,
      "/textures/studio/monitor_bottom.webp",
    );
    const monitorLeft = useLoader(
      TextureLoader,
      "/textures/studio/monitor_left.webp",
    );
    const monitorRight = useLoader(
      TextureLoader,
      "/textures/studio/monitor_right.webp",
    );
    const monitorBackPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/monitor_back_painted.webp",
    );
    const monitorTopPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/monitor_top_painted.webp",
    );
    const monitorBottomPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/monitor_bottom_painted.webp",
    );
    const monitorLeftPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/monitor_left_painted.webp",
    );
    const monitorRightPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/monitor_right_painted.webp",
    );
    ...
    const tvBottom = useLoader(
      TextureLoader,
      "/textures/studio/tv_bottom.webp",
    );
    const tvSide = useLoader(TextureLoader, "/textures/studio/tv_side.webp");
    const tvBackPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/tv_back_painted.webp",
    );
    const tvTopPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/tv_top_painted.webp",
    );
    const tvBottomPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/tv_bottom_painted.webp",
    );
    const tvSidePainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/tv_side_painted.webp",
    );

    // Load Phone textures (TikTok) - shell + painted
    const phoneBack = useLoader(
      TextureLoader,
      "/textures/studio/phone_back.webp",
    );
    const phoneSide = useLoader(
      TextureLoader,
      "/textures/studio/phone_side.webp",
    );
    const phoneBackPainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/phone_back_painted.webp",
    );
    const phoneSidePainted = useLoader(
      TextureLoader,
      isTouch ? dummyTex : "/textures/studio/phone_side_painted.webp",
    );
    ...
    const updatePaintState = useCallback(() => {
      ...
      if (shouldPaint) {
        ...
        const delay =
          hideDelayRef.current === undefined ? 0.05 : duration + 0.05;
